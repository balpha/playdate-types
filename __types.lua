...
--- Calls the game’s playdate.gameWillTerminate() event handler, if set, then returns to the Launcher.
---@field exitToLauncher fun()
...
--- Returns a string found by doing a lookup of `key` in the .strings file corresponding to the current system language, or `language`, if specified. If no string is found the input text is returned and a note about the missing string is printed on the console.
--- 
--- The optional `language` argument can be one of the strings en or jp, or one of the constants returned by playdate.getSystemLanguage() above.
...
--- Returns the system volume as set via the menu volume control. In particular, games can check if this is equal to zero and skip a delay waiting for inaudible audio to finish playing.
---@field getSystemVolume fun(): number
...
---@field mirrorStarted nil | (fun())
--- Called when the device is disconnected from Mirror.
---@field mirrorEnded nil | (fun())
...
--- Adds a mosaic effect to the display. Valid `x` and `y` values are between 0 and 3, inclusive. If `y` is not given, it is set to `x`.
--- 
...
---@field kSeekSet pd_seek_mode
---@field kSeekFromCurrent pd_seek_mode
---@field kSeekFromEnd pd_seek_mode
...
---@field setPattern fun(pattern: pd_pattern, x?: number, y?: number)
...
---@field drawLine (fun(x1: number, y1: number, x2: number, y2: number)) | (fun(ls: pd_lineSegment))
...
---@field setClipRect (fun(x: number, y: number, width: number, height: number)) | (fun(rect: pd_rect)) | (fun(rect: pd_rect))
...
---@field setStencilPattern (fun(pattern: pd_pattern)) | (fun(row1: number, row2: number, row3: number, row4: number, row5: number, row6: number, row7: number, row8: number)) | (fun(level: number, ditherType?: pd_dither_type)) | (fun(eightRows: number[])) | (fun(pattern: pd_pattern))
...
---@field drawLocalizedText (fun(key: string, x: number, y: number, width?: number, height?: number, language?: pd_language, leadingAdjustment?: number, wrapMode?: pd_wrap_mode, alignment?: pd_text_alignment)) | (fun(key: string, rect: pd_rect, language?: pd_language, leadingAdjustment?: number))
...
---@field kWrapClip pd_wrap_mode
---@field kWrapCharacter pd_wrap_mode
---@field kWrapWord pd_wrap_mode
---@field kAlignLeft pd_text_alignment
---@field kAlignCenter pd_text_alignment
---@field kAlignRight pd_text_alignment
...
---@field setEnabled fun(flag: boolean, function: fun(string|nil))
...
---@field getStatus fun(): pd_network_status
---@field http pd_http_lib
---@field https pd_https_lib
---@field tcp pd_tcp_lib
---@field kStatusNotConnected pd_network_status
---@field kStatusConnected pd_network_status
---@field kStatusNotAvailable pd_network_status
...
---@field seek fun(self: pd_file_file, offset: number, whence?: pd_seek_mode)
...
---@field pointOnArc fun(self: pd_arc, distance: number, extend?: boolean): pd_arc
...
---@field drawFaded fun(self: pd_image, x: number, y: number, alpha: number, ditherType: pd_dither_type, flip?: pd_image_flip)
...
---@field getAlwaysRedraw fun(): boolean
...
--- 
--- Changing your sprite’s center values will change your sprite’s onscreen location. (Your sprite’s x and y values will remain as they were, but because those now represent a different location on your sprite, the sprite will be drawn in a different location. You can see this for yourself by calling sprite:getBounds() before and after your :setCenter() call: the first two returned values, representing the upper left corner of your sprite, will be different.)
...
---@field updatesEnabled fun(self: pd_sprite): boolean
...
---@field markDirty (fun(self: pd_sprite)) | (fun(self: pd_sprite, x: number, y: number, width: number, height: number)) | (fun(self: pd_sprite, rect: pd_rect))
...
---@field collisionsEnabled fun(self: pd_sprite): boolean
...
--- Sets the given image to the video render context. Future video:renderFrame() calls will draw into this image. Note that renderFrame() does not fill in the mask part of the context image, if it has a mask. In most cases, you’ll want to use a fully opaque image for the context image.
...
--- Draws the given frame into the video’s render context. Note that renderFrame() preserves the context image’s mask layer, if it has one.
---@field renderFrame fun(self: pd_video, number: integer)
--- Returns the frame number of the currently displayed frame.
---@field getCurrentFrame fun(self: pd_video): number

---@class pd_http_lib : pd_http
--- Returns a playdate.network.http object for connecting to the given server. The default port is 443 if usessl is true, otherwise 80; the default value for usessl is false. If the user has not yet given permission for the device to connect to the server, the game is paused while the system asks the user to allow or deny network access for the provided reason, if one is given. Since the system uses a coroutine yield() to show the dialog to request access (if not already given), it cannot be called from an input handler or other system callback.
---@field new fun(server: string, port?: number?, usessl?: boolean, reason?: string?): pd_http
--- playdate.network.http.new() will automatically request access if needed (and note that new() only creates an object for connecting, doesn’t open the connection until get() or post() is called) but if you want to present the access dialog ahead of time you can use this function. Notably, this lets you request access to all HTTP servers by leaving the server field empty, or all subdomains of a domain by passing in the parent. Note that this function uses a coroutine yield() to pause the runtime while the permission dialog is up, so it can’t be called from a button handler or other system callback, must be called at startup or from the playdate.update() context.
---@field requestAccess fun(server?: string?, port?: number?, usessl?: boolean?, reason?: string?)

---@class pd_http
--- Closes the HTTP connection. The connection may be used again for another request.
---@field close fun(self: pd_http)
--- If flag is true, this causes the HTTP request to include a `Connection: keep-alive` header.
---@field setKeepAlive fun(self: pd_http, flag: boolean)
--- Adds a Range: bytes header to the HTTP request.
---@field setByteRange fun(self: pd_http, from: number, to: number)
--- Sets the length of time (in seconds) to wait for the connection to the server to be made.
---@field setConnectTimeout fun(self: pd_http, seconds: number)
...
---@field get fun(self: pd_http, path: string, headers?: (string|string[]|table)?): (boolean,string|nil)
...
---@field setReadTimeout fun(self: pd_http, seconds: number): nil
--- Sets the size of the connection’s read buffer.
---@field setReadBufferSize fun(self: pd_http, bytes: number)
...
---@field setHeadersReadCallback fun(self: pd_http, function: fun())
...
---@field setRequestCompleteCallback fun(self: pd_http, function: fun())
--- Sets a function to be called when the server has closed the connection.
---@field setConnectionClosedCallback fun(self: pd_http, function: fun())

---@class pd_https_lib
--- A shortcut to make source code a little easier to read: Returns a playdate.network.http object with the usessl flag set to `true`.
---@field new fun(server: string, port?: number?, reason?: string?): pd_https

---@class pd_tcp_lib : pd_tcp
...
---@field new fun(server: string, port: number, usessl?: boolean?, reason?: string?): pd_tcp
--- playdate.network.tcp.new() will automatically request access if needed (and note that new() only creates an object for connecting, doesn’t open the connection until open() is called) but if you want to present the access dialog ahead of time you can use this function. Notably, this lets you request access to all servers by leaving the server field empty, or all subdomains of a domain by passing in the parent. Access to all ports on a given server can be requested by leaving port empty. Note that this function uses a coroutine yield() to pause the runtime while the permission dialog is up, so it can’t be called from a button handler or other system callback, must be called at startup or from the playdate.update() context.
---@field requestAccess fun(server?: string?, port?: number?, reason?: string?)

---@class pd_tcp
--- Sets the length of time (in seconds) to wait for the connection to the server to be made.
---@field setConnectTimeout fun(self: pd_tcp, seconds: number)
...
---@field setReadTimeout fun(self: pd_tcp, seconds: number): nil
--- Sets the size of the connection’s read buffer.
---@field setReadBufferSize fun(self: pd_tcp, bytes: number)
...
---@field read fun(self: pd_tcp, length?: number): string
...
---@field write fun(self: pd_tcp, data: string): boolean
--- Returns the number of bytes passed to the network device but still waiting to be sent.
---@field getSentBytesPending fun(self: pd_tcp): number
--- Returns a text description of the last error on the connection, or nil if no error occurred.
---@field getError fun(self: pd_tcp): string|nil
--- Sets a function to be called when the server has closed the connection.
---@field setConnectionClosedCallback fun(self: pd_tcp, function: fun())
...
---@field setPaused fun(self: pd_fileplayer, paused: boolean)
...
---@field decompress fun(self: pd_sample): boolean
...
---@field getDryLevelSignal fun(self: pd_channel): pd_signal
--- Returns a signal that follows the volume of the channel after effects are applied.
---@field getWetLevelSignal fun(self: pd_channel): pd_signal
--- Returns a source used to route the channel’s output, after all of the channel’s source and effects are rendered but before channel volume and pan are applied, into another channel.
---@field getOutputAsSource fun(self: pd_channel): pd_source
...
---@field clearEnvelope fun(self: pd_synth)
...
---@field setWavetable fun(self: pd_synth, sample: pd_sample, samplesize: integer, xsize: integer, ysize?: integer)
...
---@field getValue fun(self: pd_signal): number
...
--- Sets the center value of the LFO.
---@field setOffset fun(self: pd_lfo, center: number)
--- Sets the depth of the LFO’s modulation.
---@field setDepth fun(self: pd_lfo, depth: number)
--- Sets the depth of the LFO’s modulation.
---@field setScale fun(self: pd_lfo, depth: number)
...
---@field setStartPhase fun(self: pd_lfo, phase: number)
--- Sets the the LFO’s random number generator seed to value, allowing for reproducible sequences in LFOs that have random elements (currently only kLFOSampleAndHold). The given value will be truncated to an unsigned 16-bit integer.
---@field setRandomSeed fun(self: pd_lfo, value: number)
...
---@field getValue fun(self: pd_lfo): number
...
---@field getValue fun(self: pd_envelope): number
...
--- When flag is false the amplitude quantizing is linear, by simply clearing a fixed number of low-order bits in each sample. When true, the amount of quantizing is proportional to the amplitude of the sample so that quieter sounds don’t get completely lost.
---@field setExponential fun(self: pd_bitcrusher, flag: boolean)
--- Sets the amount of amplitude quantizing to `amt`. Valid values are 0 (no effect) to 1 (quantizing output to 1-bit).
---@field setDepth fun(self: pd_bitcrusher, amt: number)
--- Sets a signal to modulate the filter depth. Set to `nil` to clear the modulator.
---@field setDepthMod fun(self: pd_bitcrusher, signal: pd_signal?)
--- Sets the amount of sample rate reduction; 0 is no undersampling, 1 reduces the sample rate so much the audio stops playing.
---@field setDownsampling fun(self: pd_bitcrusher, amt: number)
--- Sets a signal to modulate the sample rate reduction. Set to `nil` to clear the modulator.
---@field setDownsamplingMod fun(self: pd_bitcrusher, signal: pd_signal?)
...
---@field new fun(midi_path?: string): pd_sequence
...
--- Set multiple notes at once, each array element should be a table containing values for the keys step, note, length, and velocity.
...
---@class pd_instrument
--- Adds the given playdate.sound.synth to the instrument’s voice pool. If a note argument is given, the voice is only used for that note, and is played at C4 (i.e. rate=1.0 for samples). If a range of notes is given, the note at rangeStart is played at C4 and notes up to rangeEnd (inclusive) increase by half steps. The note, rangeStart, and rangeEnd arguments can be MIDI note numbers or note names like "Db3". The final transpose argument transposes the notes played, in half-tone units.
--- 
--- When a note is played on the instrument, it uses the first voice that’s not currently playing; if no voices are free it uses the voice that’s been off (i.e. released) the longest, and if no voices are released it uses the voice that’s been playing the longest. See synth:copy() for a convenient way to add multiple voices to allow for polyphony.
---@field addVoice (fun(self: pd_instrument, v: pd_synth, note?: string|number)) | (fun(self: pd_instrument, v: pd_synth, rangeStart?: string|number, rangeEnd?: string|number, transpose?: number))
--- Sets the pitch bend to be applied to the voices in the instrument, as a fraction of the full range.
---@field setPitchBend fun(self: pd_instrument, amount: number)
--- Sets the pitch bend range for the voices in the instrument. The default range is 12, for a full octave.
---@field setPitchBendRange fun(self: pd_instrument, halfsteps: number)
...
--- Sends a stop signal to all playing notes, with an optional trigger time (default is the current time).
---@field allNotesOff fun(self: pd_instrument, when?: number)
--- An alias for playdate.sound.instrument:allNotesOff([when]), to match the other sound source types.
---@field stop fun(self: pd_instrument, when?: pd_UNKNOWN)
--- Returns true if any voice in the instrument is currently playing, otherwise false.
---@field isPlaying fun(self: pd_instrument): boolean
...
---@field setScale fun(self: pd_controlsignal, scale: number)
--- Sets the offset value for the control signal.
---@field setOffset fun(self: pd_controlsignal, offset: number)
--- Returns the current output value of the control signal.
---@field getValue fun(self: pd_controlsignal): number

---@class pd_micinput_lib
--- playdate.sound.micinput.recordToSample() will automatically request access if needed but if you want to present the access dialog ahead of time you can use this function. Note that this function uses a coroutine yield() to pause the runtime while the permission dialog is up, so it can’t be called immediately at startup or from a button press handler; it must be called from within the playdate.update() context.
---@field requestAccess fun(reason?: string)
...
--- 
--- If the system needs to ask the user for permission to listen on the microphone it will present a dialog, using the optional purpose string to explain the reason for recording. If it’s preferable to show the permission dialog sooner, e.g. game startup, use playdate.sound.micinput.requestAccess().
---@field recordToSample fun(buffer: pd_sample, completionCallback: fun(pd_sample), purpose?: string): boolean
...
---@field startListening fun(source?: string): (boolean,string?)
...
---@field draw fun(self: pd_crankIndicator, xOffset?: number, yOffset?: number)
...
---@field resetAnimation fun(self: pd_crankIndicator)
...
---@field getBounds fun(self: pd_crankIndicator): (number, number, number, number)
...
---@field new fun(delay: number, imageTable: pd_imagetable|pd_image[], shouldLoop?: boolean): pd_loop
